    ...
    <div :class="theme.bg">
        ...
        <div class="flex justify-around flex-wrap items-center p-2">
            ...
            <div  v-if="item?.eligibilityCriteria" class="bg-gray-200 dark:bg-gray-700 rounded-xl p-2 shadow-md flex justify-center items-center flex-col space-y-1 m-2 w-full">
                <h3 class="font-light  mb-2" :class="theme['text']">Eligibility</h3>
                <p class="" v-if="item?.eligibilityCriteria?.criteriaText" v-html="item?.eligibilityCriteria?.criteriaText"></p>
                ...
                <table class="w-full text-left " v-if="item?.eligibilityCriteria?.inclusionCriteria">
                    <colgroup>
                        <col width='50%'>
                        <col width='50%'>
                    </colgroup>
            ...
            <div  v-if="item?.armGroup" class="bg-gray-200 dark:bg-gray-700 rounded-xl p-2 shadow-md  space-y-1 m-2 w-full">
                <h3 class="font-light  mb-2" :class="theme['text']">Intervention</h3>
                <ul>
                    <li v-for="(group, i) in item?.armGroup" :key="i + 'ag'" class="mb-2">
                        <details>
                            <summary><p class="font-bold inline cursor-pointer text-gray-800 dark:text-white">{{group?.name}}</p></summary>
                            <p>{{group?.description}}</p>
                            <div class="space-y-1">
                                <div v-for="(inter, it) in group?.intervention" :key="it + 'inter'" class="bg-white dark:bg-gray-800 rounded-lg p-2 ">
                                    <h5 class="font-bold">{{inter?.name}}</h5>
